...
get_players_in_range(entt::registry& r, const entt::entity& e)
{
  ...
  const auto entity = get_within_range<TeamComponent>(r, e, 1000 * 1000);

  std::vector<std::pair<entt::entity, int>> players;
  for (const auto& [entity, range] : entity) {
    const auto& team = r.get<TeamComponent>(entity);
    if (team.team == AvailableTeams::player)
      players.push_back({ entity, range });
  }
  ...
  for (const auto& [e, e_c] : r.view<EnemyComponent>().each()) {
    ...
    if (turn_state.do_shoot && has_moved && !has_shot) {
      if (has_destination(r, e) && at_destination(r, e)) { // wait to arrive...
        ...
        // arrived! shoot if possible!
        const auto players = get_players_in_range(r, e);
        if (players.size() > 0) {
          // set shoot target
          StaticTargetComponent new_target;
          new_target.target = get_position(r, players[0].first);
          r.emplace_or_replace<StaticTargetComponent>(e, new_target);
