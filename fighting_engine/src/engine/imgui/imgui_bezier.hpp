...
namespace ImGui
{
    ...
    vec2 EvaluateCubic(vec2 a, vec2 b, vec2 c, vec2 d, float t)
    {
        ...
        vec2 val = mix(p0, p1, t);
        return val;
    }

    template<int steps>
    void bezier_table(std::vector<vec2> P, vec2 results[steps + 1])
    {
    ...
    float BezierValue(std::vector<vec2> P, float t)
    {
        vec2 val = EvaluateCubic(P[0], P[1], P[2], P[3], t);
        return val.y;
    }
    ...
    int Bezier(const char* label, float dt, float middle_points[4], float start_point[2], float end_point[2]) {
        ...
        // eval curve
        std::vector<vec2> Q = std::vector<vec2>
        {
