...
float scanlines_opacity = 0.4;
float scanlines_width = 0.25;
float grille_opacity = 0.0;
bool pixelate = false; 
bool roll = false;
float roll_speed = 5.0; // Positive values are down, negative are up
float roll_size = 10.0;
float roll_variation = 1.8; // This valie is not an exact science. You have to play around with the value to find a look you like. How this works is explained in the code below.
float distort_intensity = 0.4; // The distortion created by the rolling effect.
float noise_opacity = 0.4;
float noise_speed = 0.0; // There is a movement in the noise pattern that can be hard to see first. This sets the speed of that movement.
float static_noise_intensity = 0.01;
float aberration = 0.0; // Chromatic aberration, a distortion on each color channel.
float brightness = 1.2; // When adding scanline gaps and grille the image can get very dark. Brightness tries to compensate for that.
...
main()
{
  ...
  vec4 col = texture(tex_to_crt, v_uv);
  vec2 uv = warp(v_uv);
  vec2 text_uv = uv;
  ...
  vec4 text;
	...
	// 	text.r = texture(tex_to_crt, text_uv + roll_uv * 0.8 + vec2(aberration, 0.0) * .1).r;
	// 	text.g = texture(tex_to_crt, text_uv + roll_uv * 1.2 - vec2(aberration, 0.0) * .1 ).g;
	// 	text.b = texture(tex_to_crt, text_uv + roll_uv).b;
	// }
	// else
	{
		// If roll is false only apply the aberration without any distorion. The aberration values are very small so the .1 is only 
		// to make the slider in the Inspector less sensitive.
		text.r = texture(tex_to_crt, text_uv + vec2(aberration, 0.0) * .1).r;
		text.g = texture(tex_to_crt, text_uv - vec2(aberration, 0.0) * .1).g;
		text.b = texture(tex_to_crt, text_uv).b;
	}
  ...
  out_colour = vec4(text.r, text.g, text.b, 1.0);
}