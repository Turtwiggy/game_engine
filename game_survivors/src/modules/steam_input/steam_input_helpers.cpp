...
init_steam_input(entt::registry& r)
{
  ...
  create_persistent<SINGLE_SteamMappings>(r);
  create_persistent<SINGLE_SteamConnectedControllers>(r);
...
{
  auto& controllers = get_first_component<SINGLE_SteamConnectedControllers>(r);
  controllers.n_active = SteamInput()->GetConnectedControllers(controllers.handles.data());
};

void
generate_button_state(const SINGLE_SteamMappings& mappings_c,
                      SINGLE_SteamConnectedControllers& connected_c,
                      InputHandle_t handle)
{
  const auto you_held = connected_c.last_frame_held[handle]; // copy

  // set all as unheld
  connected_c.last_frame_held[handle].clear();
  ...
  for (int i = 0; i < static_cast<int>(DA::count); i++) {
    ...
    const bool held = controller_button_held(mappings_c, handle, act);
    ...
    if (held)
      connected_c.last_frame_held[handle].push_back(act);
  }

  connected_c.this_frame_down[handle] = newly_down;
  connected_c.this_frame_release[handle] = newly_released;
};
...
update_steam_input(entt::registry& r)
{
  ...
  auto& steam_mappings_c = get_first_component<SINGLE_SteamMappings>(r);
  ...
  steam_connected_c.this_frame_down.clear();
  for (int h = 0; h < steam_connected_c.n_active; h++)
    generate_button_state(steam_mappings_c, steam_connected_c, steam_connected_c.handles[h]);

  set_all_steam_controller_action_set(steam_mappings_c, steam_connected_c, AS::ActionSet_GameControls);

  // Update the global handles every frame that process input for ui
  auto& game_joined_c = get_first_component<SINGLE_SteamControllerGameState>(r);
  ...
  global_input_c.handles = game_joined_c.handles_that_want_to_play;
  // global_input_c.handles = steam_connected_c.handles;
};

bool
controller_button_down(const SINGLE_SteamConnectedControllers& connected_c, InputHandle_t handle, const DA dwAction)
{
  if (handle == 0)
    return false;
  const std::unordered_map<InputHandle_t, std::vector<DA>>& all_down = connected_c.this_frame_down;
...
bool
controller_button_held(const SINGLE_SteamMappings& steam_c, const InputHandle_t handle, const DA dwAction)
{
  if (handle == 0)
    return false;
...
controller_button_release(const SINGLE_SteamConnectedControllers& connected_c, const InputHandle_t handle, const DA dAction)
{
  if (handle == 0)
    return false;
  const std::unordered_map<InputHandle_t, std::vector<DA>>& all_release = connected_c.this_frame_release;
...
void
set_steam_controller_action_set(const SINGLE_SteamMappings& steam_c, const InputHandle_t handle, const AS set)
{
  if (handle == 0)
    return;
...
set_all_steam_controller_action_set(const SINGLE_SteamMappings& mappings_c,
                                    ...
                                    const AS set)
{
  for (int j = 0; j < connected_c.n_active; j++) {
    auto handle = connected_c.handles[j];
    set_steam_controller_action_set(mappings_c, handle, set);
  }
};
