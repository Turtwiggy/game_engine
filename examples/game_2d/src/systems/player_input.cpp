...
game2d::update_player_input_system(entt::registry& registry, engine::Application& app)
{
...

  {
    ...
    view.each([&registry, &app, &adjusted_mouse_pos, &grid_size](const auto& player, auto& pos) {
      //
  ...
  // Update cursor
  {
    ...
    view.each([&adjusted_mouse_pos, &grid_size](const auto entity, const auto& c, auto& pos) {
      glm::ivec2 grid_slot = engine::grid::world_space_to_grid_space(adjusted_mouse_pos, grid_size);
