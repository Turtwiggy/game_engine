...
//
////In Update Function:
// ImGui::SliderFloat("Bezier Sample Value", &sample_x, 0.0f, 1.0f, "ratio =
// %.3f"); ImGui::SliderFloat2("Bezier Start Point", start_point, 0.f, 1.f,
// "ratio = %.3f"); ImGui::SliderFloat2("Bezier End Point", end_point, 0.f, 1.f,
// "ratio = %.3f"); ImGui::Bezier("Bezier Control", sample_x, middle_points,
...
namespace ImGui {
glm::vec2
EvaluateQuadratic(glm::vec2 a, glm::vec2 b, glm::vec2 c, float t)
{
...
glm::vec2
EvaluateCubic(glm::vec2 a, glm::vec2 b, glm::vec2 c, glm::vec2 d, float t)
{
...
template<int steps>
void
bezier_table(std::vector<glm::vec2> P, glm::vec2 results[steps + 1])
{
...
float
BezierValue(std::vector<glm::vec2> P, float t)
{
...
int
Bezier(const char* label,
       float dt,
       float middle_points[4],
       float start_point[2],
       float end_point[2])
{
  // visuals
  ...
  RenderFrame(
    bb.Min, bb.Max, GetColorU32(ImGuiCol_FrameBg, 1), true, Style.FrameRounding);

  // background grid
  for (int i = 0; i <= Canvas.x; i += (Canvas.x / 4)) {
    DrawList->AddLine(Imglm::vec2(bb.Min.x + i, bb.Min.y),
                      Imglm::vec2(bb.Min.x + i, bb.Max.y),
                      GetColorU32(ImGuiCol_TextDisabled));
  }
  for (int i = 0; i <= Canvas.y; i += (Canvas.y / 4)) {
    DrawList->AddLine(Imglm::vec2(bb.Min.x, bb.Min.y + i),
  ...
  std::vector<glm::vec2> Q =
    std::vector<glm::vec2>{ { start_point[0], start_point[1] },
  ...
  // control points: 2 lines and 2 circles
  {
    ...
    for (int i = 0; i < 2; ++i) {
      pos[i] = Imglm::vec2(middle_points[i * 2 + 0], 1 - middle_points[i * 2 + 1]) *
                 (bb.Max - bb.Min) +
               bb.Min;
    ...
    if (distance[selected] < (4 * GRAB_RADIUS * 4 * GRAB_RADIUS)) {
      SetTooltip("(%4.3f, %4.3f)",
                 middle_points[selected * 2 + 0],
                 middle_points[selected * 2 + 1]);

      if (/*hovered &&*/ (IsMouseClicked(0) || IsMouseDragging(0))) {
        float& px =
          (middle_points[selected * 2 + 0] += GetIO().MouseDelta.x / Canvas.x);
        float& py =
          (middle_points[selected * 2 + 1] -= GetIO().MouseDelta.y / Canvas.y);
  ...
  // draw curve
  {
    ...
    for (int i = 0; i < SMOOTHNESS; ++i) {
      ...
      Imglm::vec2 r(p.x * (bb.Max.x - bb.Min.x) + bb.Min.x,
                    p.y * (bb.Max.y - bb.Min.y) + bb.Min.y);
      Imglm::vec2 s(q.x * (bb.Max.x - bb.Min.x) + bb.Min.x,
  ...
  DrawList->AddCircleFilled(Imglm::vec2(devalx, 1 - devaly) * (bb.Max - bb.Min) +
                              bb.Min,
                            GRAB_RADIUS / 2,
                            ImColor(1.f, 0.f, 0.f));
  ...
  Imglm::vec2 p1 =
    Imglm::vec2(middle_points[0], 1 - middle_points[1]) * (bb.Max - bb.Min) + bb.Min;
  Imglm::vec2 p2 =
    Imglm::vec2(middle_points[2], 1 - middle_points[3]) * (bb.Max - bb.Min) + bb.Min;
