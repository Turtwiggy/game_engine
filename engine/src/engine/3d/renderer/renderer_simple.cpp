...
RendererSimple::RendererSimple(RandomState& rnd)
{
  ...
  // object_ =
  // ResourceManager::load_model("assets/models/lizard_wizard/lizard_wizard->obj",
  // "Object");
  ...
  ResourceManager::load_shader(
    "assets/shaders/", { "blinn-phong/lit.vert", "solid_colour.frag" }, "SHADER_solid");
...
void
RendererSimple::update(float delta_time,
                       FlyCamera& camera,
                       RandomState& rnd,
                       ...
                       const glm::ivec2& screen_size)
{
  ...
  glm::mat4 light_space_matrix =
    shadowmapping_pass.calculate_light_space_matrix(light_pos_);
  ...
  ResourceManager::get_shader("SHADER_solid")
    .set_mat4("view_projection", view_projection);
                             ...
                             const Shader& shader)
{
  ...
  model = glm::rotate(
    model, glm::radians(90.0f), glm::normalize(glm::vec3(0.0f, 1.0f, 0.0f)));
...
// std::array<unsigned int, 2> Renderer::hdr_colour_buffer(int width, int
// height, unsigned int hdr_fbo)
// {
...
//        GL_FLOAT, NULL); glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
...
//    rendering unsigned int attachments[2] = { GL_COLOR_ATTACHMENT0,
//    GL_COLOR_ATTACHMENT1 }; glDrawBuffers(2, attachments);
...
// s_Data->hdr_colour_buffers = hdr_colour_buffer(screen_width, screen_height,
// s_Data->hdr_fbo);
...
//    GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE); // we clamp to the edge as the blur
//    filter would otherwise sample repeated texture values!
//    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
...
//   {
//       s_Data->object_shader->setVec3("lights[" + std::to_string(i) +
//       "]->Position", s_Data->light_positions[i]);
//       s_Data->object_shader->setVec3("lights[" + std::to_string(i) +
//       "]->Color", s_Data->light_colours[i]);
//   }
...
//   s_Data->stats->DrawCalls); renderCube(s_Data->stats->DrawCalls); model =
//   glm::mat4(1.0f); model = glm::translate(model, glm::vec3(2.0f, 0.0f,
//   1->0)); model = glm::scale(model, glm::vec3(0.5f));
//   s_Data->object_shader->setMat4("model", model);
//   renderCube(s_Data->stats->DrawCalls);
//   model = glm::mat4(1.0f);
...
//   glm::normalize(glm::vec3(1->0, 0->0, 1->0))); model = glm::scale(model,
//   glm::vec3(1->25)); s_Data->object_shader->setMat4("model", model);
//   renderCube(s_Data->stats->DrawCalls);
//   model = glm::mat4(1.0f);
