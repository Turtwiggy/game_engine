...
std::vector<Mesh> MeshLoader::process_node(aiNode* aNode, const aiScene* aScene, std::string directory, bool set_default_material)
{
    ...
    {
        //TODO(Turtwiggy)
        //sprocess_node(aNode->mChildren[i], aScene, directory, set_default_material);
    }

    return meshes;
}

Mesh MeshLoader::parse_mesh(aiMesh* aMesh, const aiScene* aScene, glm::vec3& out_Min, glm::vec3& out_Max)
{
...
std::vector<Mesh> MeshLoader::load_mesh(std::string path, bool set_default_material)
{
...
std::string MeshLoader::process_path(aiString *aPath, std::string directory)
{
