...
convert_sprite_to_uv(entt::registry& r, const std::string& sprite)
{
  ...
  const glm::ivec2 wh = { frames.animation_frames[0].w, frames.animation_frames[0].h };

  const ImVec2 tl = ImVec2(((offset.x * pixels_x + 0.0f) / size_x), ((offset.y * pixels_y + 0.0f) / size_y));
  const ImVec2 br = { ((offset.x * pixels_x + pixels_x * wh.x) / size_x),
                      ((offset.y * pixels_y + pixels_y * wh.y) / size_y) };
  return { tl, br };
};
