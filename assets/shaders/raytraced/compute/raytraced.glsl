...
vec3 trace(const ray r, vec3 diffuse) {
...
void main(void) {
   ...
   */
    ...
    */
    ...
    //vec3 color = ((1.0-t)*a) + (t*b);

    vec3 visulized_normal = 0.5 * vec3(FragPos.x + 1, FragPos.y + 1, FragPos.z + 1);
    vec3 color = trace(fwd, normalize(visulized_normal));
